...
fn main() {
    ...
    println!(
        "Creating {} TIT FOR TAT agents in kinship group 0...",
        tft_count
    );
    ...
    println!(
        "Creating {} ALWAYS DEFECT agents across {} kinship groups...",
    ...
    println!(
        ...
        if avg_defector_fitness > 0.0 {
            avg_tft_fitness / avg_defector_fitness
        } else {
            0.0
        }
    );
    ...
    for group in 0..num_kinship_groups {
        ...
        let tft_in_group = group_agents
            .iter()
            .filter(|a| a.strategy_name == "TitForTat")
            .count();
        let defectors_in_group = group_agents
            .iter()
            .filter(|a| a.strategy_name == "AlwaysDefect")
            .count();

        println!(
            "  Group {}: {} agents (TFT: {}, Defect: {}), Avg fitness: {:.2}",
            group,
            group_agents.len(),
            tft_in_group,
            defectors_in_group,
    ...
    if avg_tft_fitness > avg_defector_fitness {
        ...
        println!();
        println!(
            "  Mechanism: Kinship-based preferential interaction ({}% within-group)",
        ...
        println!(
            "  - Kinship preference too low (currently {}%)",
            kinship_preference * 100.0
        );
        println!(
            "  - Too few encounters (currently {})",
            encounters_per_generation
        );
